# MobARK: Dependency & License Audit

**Project license: Apache-2.0** (see [LICENSE](../LICENSE)).

> Updated Aug 10, 2026 (M8 Phase A): the audit lists **only what MobARK
...
MobARK ships under Apache-2.0. Because the Apache-2.0 license must not be
...
permissive licenses, since they are command-line tools by design. ldid
(not GPL; saurik's original is unlicensed) was planned for M8's iOS
resign step, but iOS edit/recompile was **deferred to v1.1** at the M8
kickoff (Aug 10, 2026): it will follow the same subprocess-only rule if
it is ever added. apktool and the Android
build-tools (zipalign/apksigner, both Apache-2.0) are M8 additions and are
audited here when they land.

The library dependencies actually imported by MobARK are all permissive
...
| pydantic | 2.10.3 | MIT | |
| pydantic-settings | 2.7.0 | MIT | `Settings` (MOBARK_* env) |
...
> license-posture change). Registered as the `MobarkReport` TTF
> (`MOBARK_REPORT_FONT` default `/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf`);
...
- **Vendored MASTG semgrep rules** (`app/analysis/rules/mastg/`, 51 files):
  ...
  invokes only the local `rules/{mobark,mastg}` dirs.
...
- **M8 toolchain (Aug 10, 2026, Phase A):** apktool (Apache-2.0, 3.0.3) and
  ...
  installed in the app image at `/opt/mobark-tools/apktool` (jar + wrapper
  script) and `/opt/mobark-tools/build-tools`: both invoked as subprocesses
