...
**Follow-ups (Aug 10–11, 2026 — committed as `302ee56`; all gates green at
each step, final 644 backend tests + ruff clean, tsc + vite build green):**
- [x] **Smali-mode findings dots + anchored annotation rail** — `GET /scans/{id}/smali-mapping` also returns **anchors**: each finding's jadx line → the containing method's smali `.method` line (brace-counted `_jadx_methods` incl. one-line bodies, `<init>/<clinit>` included, constructors via class-name→`<init>`); unresolved lines stack gracefully. Mapping cache bumped to **v2** (`mapping` + `anchors`; stale v1 rebuilds). Live scan 24: 238 mappings / 172 anchor files, 31/31 sampled anchors land on `.method` lines. Frontend aliases smali finding lines to anchors (editor-path measure: textarea line-height + scrollHeight). **592 backend tests green.**
- [x] **Dependencies tab implemented + cached** — local-first inventory `GET /scans/{id}/dependencies` (`analysis/dependencies.py`): Android third-party package groups (known-library labels, app-package exclusion, per-group non-suppressed semgrep tallies, `lib/*.so` ABIs, runtime markers) · iOS linked dylibs from the LIEF profile + embedded frameworks; `DependenciesPanel.tsx` replaces the placeholder; per-dependency **Check known CVEs** pre-fills the Agent dock draft (M7 web surface). Inventory **cached per scan** (`dependencies_cache.json`, findings-fingerprint identity → suppress/restore recomputes on next GET, lazily). Live-verified in compose + suppress/restore rewrite. **635 backend tests green.**
- [x] **Dock chat is THE agent edit surface** — Decompiler's inline "✨ Ask agent to edit" bar removed; the edit conversation lives in the dock. `find_smali_sibling` agent tool (jadx `sources/…/Foo.java` → editable `smali*/…/Foo.smali`, multidex first-found); fake-model edit demo searches FIRST (content keyword from the user's question). Dock gains a **Review edits (n) pill** + auto-opens the shared review modal on a landed proposal; ✏️ hint when the decode isn't ready; proposals review lifted to DashboardView (per-scan edits list, `editVersion` remount, single `ProposalsModal`). **603 backend tests green.**
- [x] **@-mention files in the dock** — typing `@` opens a mention picker over the decompiler tree (lazy first fetch, files only, iOS binaries excluded; Arrow/Enter/Tab/Escape nav); draft mentions render as removable chips (× strips; click opens the file) and sent bubbles as clickable chips. `ChatRequest.mentioned_files` → `_load_mentioned_files` attaches current content (applied-edit overlay) as a **USER-MENTIONED FILES** system-prompt section; missing paths degrade to an inline note, deduped, capped 10/20k/60k. Fake model is mention-aware (editable mention → edit target; jadx mention → search → find_smali_sibling → propose sibling). Chip-click fix: resolver handles full tree paths. Multi-mention verified (two files: both chips render, dedup holds, each opens its own file). **613 backend tests green.**
- [x] **Plan-narration nudge + 4-line thinking clamp (owner report, Aug 11)** — a local model answered with pure plan narration and no tool call; the loop previously accepted it as the final answer. Now: content-only rounds that describe an intended tool action (and cite no `file:line`) are nudged (`_NARRATION_INTENT_RE`, `_MAX_NARRATION_NUDGES=2`) and the loop CONTINUES so the model actually runs search_code → read → rolls up a grounded answer; system prompt forbids describing actions instead of taking them; stale `final_text` cleared on nudge (loop exhaustion falls through to the grounded fallback — review catch). Streaming **thinking display clamped to 4 lines** (`.stream-text` line-clamp 4 + overflow hidden); full answer still lands in the finalized message. Fake-model demos unaffected. **644 backend tests green.**
