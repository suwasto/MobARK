...
const FILTER_ORDER: Severity[] = ['critical', 'high', 'medium', 'low', 'info']
...
function FindingRow({ scanId, finding }: { scanId: number; finding: FindingRead }) {
  ...
  return (
    <div className={`finding ${open ? 'open' : ''}`}>
      <div className={`spine ${finding.severity}`} />
...
/** Findings tab: severity filter chips + the full findings list. */
  ...
  return (
    <div>
      ...
      {!loading &&
        !error &&
        visible.map((f) => (
          <FindingRow key={f.id} scanId={scanId} finding={f} />
        ))}
    </div>
  )
}
