...
MobARK is a compose stack: `app` (FastAPI) + `worker` (RQ) + `redis` +
...
flowchart LR
    ...
    DB[("SQLite /data/mobark.db<br/>sessions.vault_wrap (MK under token)")]
    FILES[("data dir: uploads · trees · graphs")]
    VAULT[("data dir /data/users/{uid}/<br/>key_wrap.json (MK under password)<br/>stores + key blobs")]
    CTX["request context<br/>user_id · master key"]
    ...
    WORKER --> REDIS
    API --> REDIS & DB & FILES
    AUTH -->|password → KEK<br/>unlock/create MK| VAULT
    AUTH -->|MK under token| DB
    API -->|cookie + vault_wrap<br/>→ unwrap MK| DB
    API -->|per request| CTX
    CTX --> MODEL & SEARCHSVC
    MODEL -->|unwrap key blobs| VAULT
    SEARCHSVC -->|unwrap key blobs| VAULT
```
...
- **Auth is the outer gate.** `health` + `auth` are the only open
  ...
  per-user isolation is structural. `MOBARK_AUTH_ENABLED=0` restores the
  fully-open single-user mode.
- **Per-user stores + vault.** Model/search BYOK configs live under
  `data/users/<uid>/`; keys are AES-GCM blobs under a per-user master key
  (MK) that never touches disk in plaintext. The MK is wrapped under the
  login password (`key_wrap.json`, scrypt KEK), then re-wrapped under the
  session token (`sessions.vault_wrap`); each guarded request unwraps it
  from cookie + session row into the request context, which the store
  factories read to decrypt keys. OAuth accounts unlock with a dedicated
  passphrase per session. The agent loop receives `user_id` +
