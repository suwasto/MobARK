...
> ARM hosts pull the native arm64 image, no emulation needed. One gap:
> **edit & recompile** depends on Google's Android build-tools, which
> are Linux x86_64-only, so that feature is amd64-only.
