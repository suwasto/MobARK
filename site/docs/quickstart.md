...
Register an account, sign in, and upload an APK or IPA. MASA analyzes it
...
Set `MASA_AUTH_ENABLED=0` in `.env` — MASA then behaves exactly as the
...
All settings are optional and read from the `MASA_` environment prefix
...
| `MASA_AUTH_ENABLED` | `1` | `0` restores the fully-open single-user behavior (dev/CI) |
| `MASA_SESSION_DAYS` | `7` | Session lifetime (sliding — refreshed on use) |
| `MASA_COOKIE_SECURE` | `0` | `1` when serving over TLS (session cookie gets the Secure attribute) |
| `MASA_GITHUB_CLIENT_ID` / `MASA_GITHUB_CLIENT_SECRET` | — | GitHub OAuth (login page shows the button only when both are set) |
| `MASA_GOOGLE_CLIENT_ID` / `MASA_GOOGLE_CLIENT_SECRET` | — | Google OAuth (same — configured-only button) |
| `MASA_PUBLIC_BASE_URL` | `http://localhost:8000` | Public origin — OAuth `redirect_uri`s are derived from it |
